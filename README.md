...
- Open reindeer.js.
- Go to function show().
- We use **p5.js**'s function [image()](https://p5js.org/reference/#/p5/image) to draw the reindeer.
...
## 3.5. **Set reindeer position limits**
...
In the first if-statement, we check if the reindeer is at the bottom of the canvas. And if it is, we let it stay there and set the speed to 0.
In the second if-statement, we do the same, but for if the reindeer is at the top of the canvas. 
...
## 5.1. **Check if reindeer hits a pipe**
...
## 6.1. **Check if reindeer passes a pipe**
